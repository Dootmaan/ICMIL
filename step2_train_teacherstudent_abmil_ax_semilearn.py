...
attention_teacher = Attention(params.mDim).to(params.device)
...
checkpoint=torch.load('/your/path/to/model/AB-MIL_model_best_em0_resnet1024.pth',map_location='cpu')
classifier_teacher.load_state_dict(checkpoint['classifier'])
attention_teacher.load_state_dict(checkpoint['attention'])
...
# attention_student = Attention(params.mDim).to(params.device)
...
# attention_student.load_state_dict(checkpoint['attention'])
...
# testset=RandomPatchDistillationDataset('/newdata/why/CAMELYON16/',mode='test',level=1)
...
for ii in range(params.EPOCH):
    ...
    for i, data in enumerate(trainloader):
        ...
        attention_score=attention_teacher(tmidFeat_teacher,isNorm=False).squeeze(0)
    ...
    if mae<best_mae:
        best_mae=mae
        torch.save(model_student.state_dict(), 'ResNet50_teacher_student_ax_gated_semilearn_best1024.pth')
        print('new best auc, weights saved. ')
        # TestModel(testloader)
        # tsave_dict = {
